...
double IcpPointToPoint::fitStep (double *T,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active,double delta) {
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r10,r11,t0,t1) reduction(+:mum0,mum1, mut0,mut1) // schedule (dynamic,2)
  ...
  // dimensionality 3
    ...
    // establish correspondences
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2,gnum_ctr,num_ctr) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
    for (i=0; i<nact; i++) 
    {
      // kd tree query + result
      std::vector<float>         query(m_dim);
  ...
  std::cout<<"good so far1 num_ctr="<<num_ctr<<" ";
  // subtract mean
...
//// Correntropy cumputtion.//////////////////
double IcpPointToPoint::fitStepCorr (double *T,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active,double delta) {
...
double IcpPointToPoint::getResidual( double *T,const int32_t T_num,const Matrix &R,const Matrix &t,const std::vector<int> &active )
{
  ...
  {
		// extract matrix and translation vector
    std::cout<<"2. gnum_ctr="<<gnum_ctr<<" nact="<<nact;
